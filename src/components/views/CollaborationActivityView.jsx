...
export const CollaborationActivityView = ({ setActiveTab }) => {
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative z-10 space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-slate-900/80 hover:bg-slate-800 text-amber-400 border border-slate-800 text-xs font-bold transition-all cursor-pointer group"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
            <div className="glass-card p-4 rounded-2xl border-slate-800 bg-slate-900/70 space-y-1">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Total Aktivitas</span>
            ...
            <div className="glass-card p-4 rounded-2xl border-slate-800 bg-slate-900/70 space-y-1">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Selesai Sempurna</span>
              <div className="flex items-center gap-2">
                <Sparkle className="w-5 h-5 text-blue-400" />
                <span className="text-2xl font-black text-blue-300">{completedCount}</span>
              </div>
            </div>
            ...
            <div className="glass-card p-4 rounded-2xl border-slate-800 bg-slate-900/70 space-y-1">
              ...
              <div className="flex items-center gap-2">
                <RocketLaunch className="w-5 h-5 text-emerald-400" />
                <span className="text-2xl font-black text-emerald-300">{activeCount}</span>
              </div>
            </div>

            <div className="glass-card p-4 rounded-2xl border-slate-800 bg-slate-900/70 space-y-1">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block">Menunggu Respons</span>
              <div className="flex items-center gap-2">
      ...
      <div className="glass-card p-5 rounded-2xl border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
        {/* Search Bar */}
        <div className="relative flex-1 max-w-lg">
          <MagnifyingGlass className="w-5 h-5 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
          <input
            type="text"
            value={searchQuery}
        ...
        {/* Status Filter Tabs */}
        <div className="flex items-center bg-slate-900/90 p-1 rounded-xl border border-slate-800 w-fit self-start md:self-auto overflow-x-auto max-w-full">
          <button
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap flex items-center gap-1 ${
          ...
          {filteredActivities.map((act) => {
            ...
            return (
              ...
              >
                ...
                {/* Title */}
                <div>
                  <h3 className="text-xl font-black text-white tracking-tight">{act.project_title}</h3>
                </div>
                ...
                {rateDetails.isRateCard ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-1">
                    ...
                    <div className="glass-card p-3.5 rounded-2xl border-slate-800/90 bg-slate-900/60 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-sm font-extrabold text-amber-300">{act.influencer_name || 'Influencer KOL'}</span>
                      </div>
                    </div>
                    ...
                    <div className="glass-card p-3.5 rounded-2xl border-slate-800/90 bg-slate-900/60 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-sm font-extrabold text-emerald-400">{rateDetails.budgetRange || formatBudgetDisplay(act.budget)}</span>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                    <div className="glass-card p-3.5 rounded-2xl border-slate-800/90 bg-slate-900/60">
                      ...
                      <strong className="text-sm font-extrabold text-purple-300">{act.brand_name || 'Brand UMKM'}</strong>
                    </div>
                    <div className="glass-card p-3.5 rounded-2xl border-slate-800/90 bg-slate-900/60">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Influencer KOL</span>
                      <strong className="text-sm font-extrabold text-amber-300">{act.influencer_name}</strong>
                    </div>
                    <div className="glass-card p-3.5 rounded-2xl border-slate-800/90 bg-slate-900/60">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Budget Kampanye</span>
                      <strong className="text-sm font-extrabold text-emerald-400">{formatBudgetDisplay(act.budget)}</strong>
                    </div>
                  </div>
                )}
                ...
                <div className="pt-4 border-t border-slate-800/80 flex flex-wrap items-center justify-between gap-4">
                  ...
                  {setActiveTab && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-600/20 hover:bg-purple-600/30 text-purple-300 border border-purple-500/30 text-xs font-bold transition-all cursor-pointer"
                    >
      ...
      <div className="glass-card p-8 rounded-3xl border-slate-800 bg-gradient-to-r from-purple-950/40 via-slate-900 to-indigo-950/40 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="space-y-2 text-center md:text-left">
          <h3 className="text-xl font-bold text-white">Ingin Memulai Kolaborasi Baru?</h3>
        ...
        {setActiveTab && (
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => setActiveTab('marketplace')}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-xs shadow-lg hover:scale-105 transition-all"
            >
              <UsersThree className="w-4 h-4" /> Marketplace KOL
            </button>
            <button
              onClick={() => setActiveTab('products')}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold text-xs border border-slate-700 transition-all"
            >
